...
from ards_ml.training.train import load_data, split_data, train_baseline, train_xgb, youden_threshold, calibrated_proba
from ards_ml.training.evaluate import (
...
from ards_ml.explainability.shap_explainer import build_explainer, save_explainer

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
log = logging.getLogger(__name__)

DATA_PATH = "ards_ml/data/ARDS_Dataset.csv"
OUT_DIR = Path("ards_ml/artefacts")
N_TRIALS = 50
...
def main():
    ...
    config_path = Path("ards_ml/config/threshold_config.yaml")
    ...
    # Subgroup analysis — needs raw X_test (pre-pipeline features)
    subgroup_df = subgroup_analysis(X_test.reset_index(drop=True),
                                    y_test.reset_index(drop=True),
                                    y_prob_test, threshold)
